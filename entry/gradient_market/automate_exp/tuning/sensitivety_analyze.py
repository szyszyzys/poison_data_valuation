...
# Define what "usable" means, relative to the IID baseline
...
def parse_scenario_name_robust(name: str) -> Dict[str, str]:
    """
    Parses 'step4_sens_fltrust_with_attack_noniid_cifar10'
    """
    try:
        parts = name.split('_')
        if len(parts) < 7:
        ...
        return {
            "defense": parts[2],
            "attack_state": "_".join(parts[3:-2]),  # Handles "no_attack" and "with_attack"
            "data_setting": parts[-2],
            "dataset": parts[-1],
            "scenario": name,
        }
        ...
        try:
            ...
            record = {
                ...
                **hparam_info,
                ...
                "backdoor_asr": metrics.get("backdoor_asr"),
            }
...
def analyze_sensitivity(raw_df: pd.DataFrame):
    ...
    group_cols = ["scenario", "defense", "attack_state", "data_setting", "dataset",
                  "optimizer", "lr", "epochs"]

    # Handle cases where some metrics might be missing
    numeric_cols = ["test_acc", "backdoor_asr"]
    hp_agg_df = raw_df.groupby(group_cols)[numeric_cols].mean().reset_index()
    ...
    # Determine if this HP combo is "usable"
    hp_agg_df['is_usable'] = hp_agg_df['relative_perf'] >= USABLE_THRESHOLD

    # --- 3. Aggregate HP stats to get final "Cost" metrics ---
    scenario_group_cols = ["defense", "attack_state", "data_setting", "dataset"]

    # Get max, mean, std of test_acc
    cost_df = hp_agg_df.groupby(scenario_group_cols)['test_acc'].agg(
        max_test_acc='max',
        avg_test_acc='mean',
        std_test_acc='std',
        total_hp_combos='count'
    ).reset_index()

    # Get "usable" count
    usable_counts_df = hp_agg_df.groupby(scenario_group_cols)['is_usable'].agg(
        usable_hp_count='sum'
    ).reset_index()
