import pandas as pd
import json
import re
import seaborn as sns
import matplotlib.pyplot as plt
import numpy as np
import os
from pathlib import Path
from typing import List, Dict, Any, Tuple

# --- Configuration ---
BASE_RESULTS_DIR = "./results"
FIGURE_OUTPUT_DIR = "./figures/step7_martfl_only_figures"


def parse_scenario_name(scenario_name: str) -> Dict[str, Any]:
    """
    Robustly parses scenario names, handling underscores in threat models
    (e.g., 'black_box') correctly.
    """
    try:
        # 1. Handle Baseline
        if scenario_name.startswith('step7_baseline_no_attack_'):
            # Pattern: step7_baseline_no_attack_{defense}_{dataset}
            match = re.search(r'step7_baseline_no_attack_([a-zA-Z0-9]+)_(.*)', scenario_name)
            if match:
                return {
                    "threat_model": "baseline",
                    "adaptive_mode": "N/A",
                    "defense": match.group(1),
                    "dataset": match.group(2),
                    "threat_label": "0. Baseline (No Attack)"
                }

        # 2. Handle Adaptive Attacks
        elif scenario_name.startswith('step7_adaptive_'):
            # Remove prefix to parse the rest manually
            rest = scenario_name.replace("step7_adaptive_", "")

            # Identify Threat Model explicitly to avoid Regex confusion with underscores
            if rest.startswith("black_box_"):
                threat_model = "black_box"
                rest = rest[len("black_box_"):]
                threat_label = "1. Black-Box"
            elif rest.startswith("gradient_inversion_"):
                threat_model = "gradient_inversion"
                rest = rest[len("gradient_inversion_"):]
                threat_label = "2. Grad-Inversion"
            elif rest.startswith("oracle_"):
                threat_model = "oracle"
                rest = rest[len("oracle_"):]
                threat_label = "3. Oracle"
            else:
                print(f"Warning: Unknown threat model in '{scenario_name}'")
                return {"defense": "unknown"}

            # Now 'rest' should look like: {adaptive_mode}_{defense}_{dataset}
            # We assume adaptive_mode is either 'data_poisoning' or 'gradient_manipulation'
            if rest.startswith("data_poisoning_"):
                adaptive_mode = "data_poisoning"
                rest = rest[len("data_poisoning_"):]
            elif rest.startswith("gradient_manipulation_"):
                adaptive_mode = "gradient_manipulation"
                rest = rest[len("gradient_manipulation_"):]
            else:
                print(f"Warning: Unknown adaptive mode in '{scenario_name}'")
                return {"defense": "unknown"}

            # The remainder is {defense}_{dataset}
            # We split by the FIRST underscore to separate defense from dataset
            parts = rest.split('_', 1)
            if len(parts) == 2:
                defense = parts[0]
                dataset = parts[1]
                return {
                    "threat_model": threat_model,
                    "adaptive_mode": adaptive_mode,
                    "defense": defense,
                    "dataset": dataset,
                    "threat_label": threat_label
                }

        return {"defense": "unknown"}
    except Exception as e:
        print(f"Warning: Could not parse scenario name '{scenario_name}': {e}")
        return {"defense": "unknown"}


def collect_all_results(base_dir: str) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    all_seller_dfs = []
    all_global_log_dfs = []
    all_summary_rows = []

    base_path = Path(base_dir)
    print(f"Searching for 'step7_*' directories in {base_path.resolve()}...")

    # Find all scenario folders
    scenario_folders = list(base_path.glob("step7_*"))
    if not scenario_folders:
        print("Error: No 'step7_*' directories found in ./results")
        return pd.DataFrame(), pd.DataFrame(), pd.DataFrame()

    print(f"Found {len(scenario_folders)} 'step7_*' base directories.")

    for scenario_path in scenario_folders:
        scenario_params = parse_scenario_name(scenario_path.name)
        if "defense" not in scenario_params or scenario_params["defense"] == "unknown":
            continue

        # USE RGLOB to find the deeply nested final_metrics.json
        marker_files = list(scenario_path.rglob('final_metrics.json'))

        if not marker_files:
            print(f"  [SKIP] No 'final_metrics.json' found inside: {scenario_path.name}")
            continue

        print(f"  [LOAD] Found {len(marker_files)} runs in: {scenario_path.name}")

        for final_metrics_file in marker_files:
            run_dir = final_metrics_file.parent
            seed_id = f"{scenario_path.name}__{run_dir.name}"

            # Load Seller Metrics
            seller_file = run_dir / 'seller_metrics.csv'
            df_seller = pd.DataFrame()
            if seller_file.exists():
                try:
                    df_seller = pd.read_csv(seller_file, on_bad_lines='skip')
                    if not df_seller.empty:
                        df_seller['seed_id'] = seed_id
                        df_seller = df_seller.assign(**scenario_params)
                        df_seller['seller_type'] = df_seller['seller_id'].apply(
                            lambda x: 'Adversary' if str(x).startswith('adv_') else 'Benign'
                        )
                        all_seller_dfs.append(df_seller)
                except Exception as e:
                    pass

            # Load Training Log
            log_file = run_dir / 'training_log.csv'
            if log_file.exists():
                try:
                    use_cols = ['round', 'val_acc']
                    df_log = pd.read_csv(log_file, usecols=lambda c: c in use_cols, on_bad_lines='skip')
                    if 'val_acc' in df_log.columns:
                        df_log['seed_id'] = seed_id
                        df_log = df_log.assign(**scenario_params)
                        all_global_log_dfs.append(df_log)
                except Exception as e:
                    pass

            # Load Summary
            try:
                with open(final_metrics_file, 'r') as f:
                    final_metrics = json.load(f)

                adv_sel_rate = 0.0
                ben_sel_rate = 0.0
                if not df_seller.empty:
                    df_seller_last = df_seller[df_seller['round'] == df_seller['round'].max()]
                    if not df_seller_last.empty:
                        adv_sel_rate = df_seller_last[df_seller_last['seller_type'] == 'Adversary']['selected'].mean()
                        ben_sel_rate = df_seller_last[df_seller_last['seller_type'] == 'Benign']['selected'].mean()

                summary_row = {
                    **scenario_params,
                    'seed_id': seed_id,
                    'acc': final_metrics.get('acc', 0),
                    'adv_sel_rate': adv_sel_rate,
                    'ben_sel_rate': ben_sel_rate
                }
                all_summary_rows.append(summary_row)
            except Exception:
                pass

    df_seller_ts = pd.concat(all_seller_dfs, ignore_index=True) if all_seller_dfs else pd.DataFrame()
    df_global_ts = pd.concat(all_global_log_dfs, ignore_index=True) if all_global_log_dfs else pd.DataFrame()
    df_summary = pd.DataFrame(all_summary_rows)

    return df_seller_ts, df_global_ts, df_summary


# --- PLOTTING FUNCTIONS (Keep your existing ones, condensed here for brevity) ---
def plot_selection_rate_curves(df, output_dir):
    if df.empty: return
    # (Same logic as your previous code)
    print("Generating Plot 1: Selection Rate Learning Curves...")
    df_agg = df.groupby(['seed_id', 'seller_type', 'defense', 'threat_label', 'adaptive_mode', 'round'])[
        'selected'].mean().reset_index()
    df_agg['rolling_sel_rate'] = df_agg.groupby(['seed_id', 'seller_type', 'defense', 'threat_label', 'adaptive_mode'])[
        'selected'].transform(lambda x: x.rolling(3, min_periods=1).mean())

    for defense in df_agg['defense'].unique():
        for threat in df_agg['threat_label'].unique():
            df_facet = df_agg[(df_agg['defense'] == defense) & (df_agg['threat_label'] == threat)]
            if df_facet.empty: continue

            plt.figure(figsize=(10, 6))
            sns.lineplot(data=df_facet, x='round', y='rolling_sel_rate', hue='seller_type', style='adaptive_mode',
                         palette={'Adversary': 'red', 'Benign': 'blue'}, errorbar=('ci', 95))
            plt.title(f'Selection Rate: {defense.upper()} vs {threat}');
            plt.ylim(-0.05, 1.05)
            plt.savefig(output_dir / f"plot1_sel_rate_{defense}_{threat.replace(' ', '')}.png", bbox_inches='tight')
            plt.close('all')


def plot_final_summary(df, output_dir):
    if df.empty: return
    print("Generating Plot 3: Final Summary...")
    df_melted = df.melt(id_vars=['defense', 'threat_label', 'adaptive_mode'],
                        value_vars=['adv_sel_rate', 'ben_sel_rate', 'acc'], var_name='Metric', value_name='Value')
    df_melted['Value'] *= 100

    for defense in df['defense'].unique():
        df_defense = df_melted[df_melted['defense'] == defense]
        if df_defense.empty: continue
        g = sns.catplot(data=df_defense, kind='bar', x='threat_label', y='Value', col='Metric', hue='adaptive_mode',
                        height=4, aspect=1.0, errorbar=('ci', 95), sharey=False)
        g.fig.suptitle(f'Summary: {defense}', y=1.03)
        plt.savefig(output_dir / f"plot3_final_summary_{defense}.png", bbox_inches='tight')
        plt.close('all')


def main():
    output_dir = Path(FIGURE_OUTPUT_DIR)
    os.makedirs(output_dir, exist_ok=True)

    # 1. Load Data
    df_seller_ts, df_global_ts, df_summary = collect_all_results(BASE_RESULTS_DIR)

    if df_summary.empty:
        print("❌ Error: No data loaded. Check your paths.")
        return

    # 2. Filter for Martfl
    print("Filtering for 'martfl'...")
    df_martfl_ts = df_seller_ts[df_seller_ts['defense'] == 'martfl']
    df_martfl_summary = df_summary[df_summary['defense'] == 'martfl']

    if df_martfl_summary.empty:
        print("❌ Error: Data loaded, but no 'martfl' defense found.")
        return

    # 3. Plot
    plot_selection_rate_curves(df_martfl_ts, output_dir)
    plot_final_summary(df_martfl_summary, output_dir)

    print(f"✅ Done. Figures saved to {output_dir.resolve()}")


if __name__ == "__main__":
    main()