...
def load_valuation_data_from_csv(run_dir: Path) -> pd.DataFrame:
    """
    Scans seller_metrics.csv for valuation columns (influence, shapley, loo).
    Returns a DataFrame summarized by Seller Type (Benign/Adv).
    """
    ...
    try:
        ...
        # We use mean() to handle NaNs (sparse recording frequency)
