...
def gradient_inversion_attack(target_gradient, model, input_shape, num_classes, device, num_images=1, iterations=500,
    ...
    if label_type == 'optimize':
        dummy_labels_param = torch.randn(num_images, num_classes, device=device,
                                         requires_grad=True); params_to_optimize = [dummy_data, dummy_labels_param]
    else:
    ...
    for it in range(iterations):
        ...
        model.zero_grad()
        if label_type == 'optimize':
            current_labels_prob = F.softmax(dummy_labels_param, dim=-1); current_labels_idx = torch.argmax(
                current_labels_prob, dim=-1); output = model(dummy_data); task_loss = criterion(output,
                                                                                                current_labels_idx)
        else:
            current_labels_idx = fixed_labels; output = model(dummy_data); task_loss = criterion(output,
                                                                                                 current_labels_idx)
        ...
        if loss_type == 'l2':
            ...
            flat_dummy = torch.cat([g.reshape(-1) for g in dummy_gradient]); flat_target = torch.cat(
                [g.reshape(-1) for g in target_gradient]); grad_loss = 1 - F.cosine_similarity(flat_dummy, flat_target,
                                                                                               dim=0, eps=1e-8)
        else:
            raise ValueError(f"Unknown loss_type: {loss_type}")
        tv_loss = 0
        if regularization_weight > 0: diff1 = dummy_data[:, :, :, :-1] - dummy_data[:, :, :, 1:]; diff2 = dummy_data[:,
                                                                                                          :, :-1,
                                                                                                          :] - dummy_data[
                                                                                                               :, :, 1:,
                                                                                                               :]; tv_loss = torch.norm(
            diff1) + torch.norm(diff2)
        ...
        if return_best and current_loss < best_loss: best_loss = current_loss; best_dummy_data = dummy_data.detach().clone();
        if label_type == 'optimize':
    ...
    if return_best:
        final_labels = best_dummy_labels if best_dummy_labels is not None else torch.argmax(
            ...
            dim=-1) if label_type == 'optimize' else fixed_labels; final_images = best_dummy_data
    else:
        final_images = dummy_data.detach().clone(); final_labels = torch.argmax(
    ...
    for i in range(num_images):
        ...
        try:
            with warnings.catch_warnings():
                warnings.simplefilter("ignore"); current_psnr = compare_psnr(im1_single, im2_single,
                                                                             data_range=data_range)
            psnr_vals.append(current_psnr)
            if num_channels == 1:
                im1_ssim = np.squeeze(im1_single); im2_ssim = np.squeeze(im2_single); channel_axis = None
            else:
                im1_ssim = np.transpose(im1_single, (1, 2, 0)); im2_ssim = np.transpose(im2_single,
                                                                                        (1, 2, 0)); channel_axis = -1
            win_size = min(7, im1_ssim.shape[0], im1_ssim.shape[1]);
            win_size -= (1 - win_size % 2)
            if win_size >= 3:
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore"); current_ssim = compare_ssim(im1_ssim, im2_ssim,
                                                                                 data_range=data_range,
            ...
            logging.warning(f"Error calculating PSNR/SSIM for image {i}: {e}"); psnr_vals.append(
                np.nan); ssim_vals.append(np.nan)
...
def evaluate_inversion(reconstructed_images, ground_truth_images, reconstructed_labels=None, ground_truth_labels=None):
    ...
    if reconstructed_labels is not None and ground_truth_labels is not None:
        if reconstructed_labels.shape == ground_truth_labels.shape:
            correct = (
                    reconstructed_labels.cpu() == ground_truth_labels.cpu()).sum().item(); total = ground_truth_labels.numel(); label_acc = correct / total if total > 0 else np.nan
        else:
            logging.warning(
...
def visualize_and_save_attack(reconstructed_images, ground_truth_images, reconstructed_labels, ground_truth_labels,
    ...
    if num_images_to_show == 0: logging.warning("No images to visualize."); return
    ...
    try:
        plt.tight_layout(rect=[0, 0.03, 1, 0.92]); plt.savefig(save_path, dpi=300, bbox_inches='tight'); logging.info(
    ...
    try:
        ...
        if ground_truth_images is None:
            if label_type == 'ground_truth': logging.warning(
                f"GT data unavailable for {victim_id}, switching label_type to 'optimize'."); label_type = 'optimize'
        elif label_type == 'ground_truth':
            if ground_truth_labels is None or len(ground_truth_labels) != num_images:
                logging.warning(
                    f"GT labels missing/mismatch for {victim_id}, switching label_type to 'optimize'."); label_type = 'optimize'; ground_truth_labels = None
            else:
        ...
        if ground_truth_images is not None:
            ...
            if save_visuals: filename = f"round_{round_num}_victim_{victim_id}_comparison.png" if round_num is not None else f"victim_{victim_id}_comparison.png"; visualize_and_save_attack(
            ...
            if save_visuals:
                try:
                    from torchvision.utils import \
                        save_image; filename = f"round_{round_num}_victim_{victim_id}_reconstructed.png" if round_num is not None else f"victim_{victim_id}_reconstructed.png"; rec_path = save_dir / filename; save_dir.mkdir(
                        parents=True, exist_ok=True); save_image(reconstructed_images.cpu(), rec_path,
                                                                 normalize=True); logging.info(
        ...
        logging.error(f"GIA failed for victim {victim_id}: {e}", exc_info=False); attack_results_log["error"] = str(e);
