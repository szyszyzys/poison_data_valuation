...
class AdversarySeller(BaseSeller):
...
# class AdversarySeller:
#     """
#     Represents the adversary (malicious seller) attempting to infer the buyer's hidden test vectors.
#     """
#
#     def __init__(self,
#                  dataset,
...
#                  cost_variation: float = 0.5):
#         """
#         Initialize the Adversary with parameters for data manipulation.
#
#         Parameters:
...
#         self.basis_vectors = dataset
#
#         # Generate perturbations
#         self.perturbations = generate_perturbations(
#             base_vectors=self.basis_vectors,
#             n_perturb=self.n_perturb,
#             perturb_radius=self.perturb_radius
#         )
#
#         # Create synthetic dataset with multi-tier costs
#         self.X_synth, self.cost_synth = create_synthetic_dataset(
#             base_vectors=self.basis_vectors,
#             perturbations=self.perturbations,
#             n_tiers=self.n_tiers
#         )
#
#         # Store total number of unique data points
#         self.n_unique = self.X_synth.shape[0]
#
#         # For inference
#         self.inferred_x_test = None
#
#     def get_synthetic_data(self) -> Tuple[np.ndarray, np.ndarray]:
#         """
#         Returns the synthetic data and corresponding costs.
#         """
#         return self.X_synth, self.cost_synth
#
#     def manipulate_data(self):
#         """
#         Additional data manipulations can be implemented here.
#         For now, data manipulation is handled during initialization.
#         """
...
#                      lr: float = 0.01):
#         """
#         Perform inference to reconstruct the hidden test vector based on selected indices.
#
#         Parameters:
#         - chosen_indices: Indices of synthetic data points selected by the buyer.
...
#                 covariance_matrix = np.cov(self.X_synth, rowvar=False) + 1e-6 * np.eye(self.X_synth.shape[1])
#             self.inferred_x_test = reconstruct_x_test_from_selection_mahalanobis(
...
#             )
#         else:
#             raise ValueError("Unsupported scoring_method for inference.")
#
#     def evaluate_attack(self, x_test: np.ndarray) -> Tuple[float, float]:
#         """
#         Evaluate the accuracy of the inferred test vector.
#
#         Parameters:
#         - x_test: The true hidden test vector.
#
#         Returns:
#         - cosine_similarity: Cosine similarity between true and inferred vectors.
#         - mse: Mean squared error between true and inferred vectors.
#         """
#         if self.inferred_x_test is None:
#             raise ValueError("Inferred x_test is not available. Run infer_x_test first.")
#         return evaluate_reconstruction(x_test, self.inferred_x_test)
#
#     def visualize_attack(self,
#                          x_test: np.ndarray, chosen_indices,
#                          title: str = 'Advanced Selection-Only Attack (2D)'):
#         """
#         Visualize the synthetic data, selected points, true and inferred test vectors.
...
#         """
#         if self.X_synth.shape[1] != 2:
#             print("Visualization is only supported for 2D data.")
#             return
#         if self.inferred_x_test is None:
#             print("Inferred x_test is not available. Run infer_x_test first.")
#             return
#
#         plt.figure(figsize=(10, 10))
#
#         # Plot all synthetic points
#         plt.scatter(self.X_synth[:, 0], self.X_synth[:, 1], c='lightgray', label='Synthetic Data')
#
#         # Highlight chosen points
#         chosen_X = self.X_synth[chosen_indices]
#         plt.scatter(
#             chosen_X[:, 0],
#             chosen_X[:, 1],
#             c='red',
...
#         # Plot inferred x_hat
#         plt.scatter(
#             [self.inferred_x_test[0]],
#             [self.inferred_x_test[1]],
#             c='blue',
#             marker='X',
#             s=200,
...
#         plt.arrow(0, 0, self.inferred_x_test[0], self.inferred_x_test[1],
